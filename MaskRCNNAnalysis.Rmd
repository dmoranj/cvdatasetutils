...
```{r}
pathModelData <- './cvdatasetutils/models/MaskRCNNAnalysisTorre.csv'
...
modelData %>%
  group_by(id) %>%
  summarize(model_id=last(model_id), alpha=max(alpha), batch_size=last(batch_size), real_batch=last(batch_size)*last(batch_accumulator), 
            mixed=last(mixed), maxTime=max(time/3600), minTime=min(time(3600)), minLoss=min(loss)/min(batch_accumulator), maxGpu=max(gpu_total)) %>%
  arrange(minLoss)
```
